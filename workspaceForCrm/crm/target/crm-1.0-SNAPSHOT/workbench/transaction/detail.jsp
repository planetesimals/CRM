...
<head>
    ...
    <script type="text/javascript">
        ...
        function changeIcon(stage,index1) {
            ...
            //当前阶段可能性，局部刷新不会改变request域当中原本的值
            var currentPossibility = $("#possibility").html();

            //当前阶段的下标
            var index = index1;

            //前面正常阶段和后面丢失阶段的分界点下标，八大基本数据类型以及String类型才能做以下java变量转化成js变量，且要有固定的格式，尖括号和百分号必须套在双引号中
            var point = "<%=point%>";
            ...
            for(int i=0;i<dvList.size();i++){
                ...
                if("0".equals(listPossibility)){
                    ...
                    if(listStage.equals(currentStage)){
                        ...
                        //红叉-----------------------------------------
    %>
        <!--在for循环中不能用id做事情，可以使用onclick（加参，加参形式如下）-->
