...
<head>
	...
	<script>
		//把全局作用域中的pMap（java形式），转化为json形式的键值对，以下语句本身声明的就是一个json对象
		var json = {
			//以下是java脚本的形式，在jsp文件中能不出现尽量不出现，一般用EL表达式代替它，java脚本一旦在文件中出现过多，可读性差，甚至没法读，维护性差
			//在前端页面中遇到特殊需求，还是要用到java脚本的
			<%
			...
			%>
			//这一行脱离了java脚本，又能写js了，以下js中key和value的写法是硬性规定的，记住就行，把java转json了，最后的逗号会被自动屏蔽，因为本身语句声明的就是json
			"<%=key%>" : <%=value%>,
		...
		/*
			...
			关于阶段和可能性
				...
				02需求分析=20
				....
				在properties文件中，对中文支持很差，可以把中文转码，用Unicode编码  在文件：Java\jdk1.8.0_101\bin\native2ascii.exe,可以把中文解析转码
...
<form action="workbench/transaction/save.do" id="tranForm" method="post" class="form-horizontal" role="form" style="position: relative; top: -30px;">
	...
	<div class="form-group">
		...
		<div class="col-sm-10" style="width: 300px;">
			...
			<input type="hidden" name="activityId" value="01445dd23f494e1092a17374a2a6bc30"/>
		</div>
	</div>
	...
	<div class="form-group">
		...
		<div class="col-sm-10" style="width: 300px;">
			...
			<input type="hidden" name="contactsId" value="e224e21735d74edb92d442ac088fd17e"/>
		</div>
	</div>
