...
public class ClueController extends HttpServlet {
    ...
    private void convert(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException {
         ...
         */
        //如果传request，而不传createBy,在业务层中，用request.getSession().getAttribute("user")方法得出createBy
        //用request域传的话，太大了，没必要，因为request域功能强大，只是为了取createBy没必要
        //并且request和response作为请求响应的对象，应该只存在于控制器，不应该出现在业务层
        boolean flag1 = cs.convert(clueId,t,createBy);

        if(flag1){
            //如果用request域存值，一般用转发，其次基本玩的都是重定向
    ...
    private void detail(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException {
        ...
        Clue c = cs.detail(id);
        //把线索详细信息页的信息封装到request作用域中，key值为c，在前端会常用EL表达式c.id等等取值
